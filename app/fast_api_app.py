...
app.title = "daily-news-agent"
app.description = "API for interacting with the Agent daily-news-agent"
...
def healthz() -> dict:
    ...
    return {"status": "healthy", "service": "daily_news_agent"}


@app.post("/run-pipeline")
async def trigger_pipeline() -> dict:
    """Trigger the daily news pipeline execution."""
    from app.pipeline import run_daily_pipeline
    from app.agent import get_firestore_client

    db = get_firestore_client()
    results = await run_daily_pipeline(db=db)
    return results
