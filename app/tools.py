"""Tools and helpers for email validation, sanitization, and dispatch."""
...
from app.config import SENDGRID_API_KEY, SENDGRID_SENDER_EMAIL, DRY_RUN_MODE
...
def send_news_email(recipient_email: str, topic: str, html_content: str) -> Dict[str, Any]:
    """Sends news digest email via SendGrid REST API with diagnostic error logging.
    ...
    Raises:
        ...
        urllib.error.HTTPError: For retriable network/HTTP errors after handling rate limits.
    """
    ...
    try:
        ...
        if e.code in (401, 403):
            ...
            retry_header = e.headers.get("Retry-After", "10")
            try:
                retry_after = int(retry_header)
            except ValueError:
                retry_after = 15  # Fallback for HTTP-date strings
