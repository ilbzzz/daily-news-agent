...
class TestPipeline(unittest.TestCase):
  ...
  def test_calculate_next_local_8am_utc_before_8am(self):
    ...
    self.assertEqual(
        next_utc, datetime(2026, 7, 20, 12, 0, 0, tzinfo=ZoneInfo("UTC"))
    )
  ...
  def test_calculate_next_local_8am_utc_after_8am(self):
    ...
    self.assertEqual(
        next_utc, datetime(2026, 7, 21, 12, 0, 0, tzinfo=ZoneInfo("UTC"))
    )
  ...
  def test_advance_user_next_trigger_local_dst_preserving(self):
    ...
    next_trigger = advance_user_next_trigger_local(
        "America/New_York", current_trigger, now_utc
    )
    self.assertEqual(
        next_trigger, datetime(2026, 7, 21, 12, 0, 0, tzinfo=ZoneInfo("UTC"))
    )
  ...
  def test_advance_user_next_trigger_local_outage_backlog_prevention(self):
    ...
    next_trigger = advance_user_next_trigger_local(
        "America/New_York", stale_trigger, now_utc
    )
  ...
  def test_render_markdown_to_html_formatting(self):
    ...
    raw_md = (
        "# Daily Top Digest: AI\n\n- **Breakthrough**: New model released."
  ...
  def test_claim_user_transaction_success(self):
    ...
    mock_snapshot.get.side_effect = (
        lambda key: "idle" if key == "status" else None
    )
    ...
    mock_transaction.update.assert_called_once_with(
        mock_doc_ref, {"status": "processing", "updated_at": now_dt}
    )
  ...
  def test_claim_user_transaction_already_processing(self):
    ...
    mock_snapshot.exists = True
    mock_snapshot.get.side_effect = (
        lambda key: "processing" if key == "status" else None
    )
    mock_doc_ref.get.return_value = mock_snapshot
  ...
  def test_cleanup_stale_locks_batch(self):
    ...
    mock_batch.update.assert_called_once_with(
        "ref_stale", {"status": "idle", "updated_at": now_dt}
    )
  ...
  def test_run_daily_pipeline_flow(self):
    ...
    # Configure stale lock query to return empty list
    mock_db.collection.return_value.where.return_value.where.return_value.limit.return_value.get.return_value = (
        []
    )

    # Configure due user mock document
    ...
    mock_due_doc.to_dict.return_value = {
        ...
        "timezone": "America/New_York",
        "next_trigger_utc": datetime(
            2026, 7, 20, 12, 0, 0, tzinfo=timezone.utc
        ),
        "status": "idle",
        "active": True,
    }
    # Configure due query mock return
    mock_db.collection.return_value.where.return_value.where.return_value.where.return_value.limit.return_value.get.return_value = [
        mock_due_doc
    ]
